...
function User({ user }) {
  //Extraerlos valores
  const {Id_user, name, lastname, alias, role } = user;
  ...
  return (
    <Fragment>
      <tr className="text-center">
        ...
        <td>{role}</td>
        <td>
          <Link to={`/users/edit/${Id_user}`} className="btn btn-info"><i className="fas fa-edit w-100 fa-lg
          "></i></Link>
        </td>
        <td>
          <button className="btn btn-danger"
                  onClick={() => deleteUser(Id_user)}>
            <i className="fas fa-trash w-100 fa-lg "></i>
          </button>
        </td>
      </tr>
    </Fragment>
